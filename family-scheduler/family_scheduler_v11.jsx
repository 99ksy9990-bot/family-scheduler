...
const C = {
  ...
  white:"#FFFFFF",  page:"#F5F7FA",
};

// Per-tab accent — drives every header gradient & active color
const TAB_ACCENT = {
  home:    {from:"#38BDF8", to:"#818CF8", active:C.sky},
  calendar:{from:"#38BDF8", to:"#6366F1", active:C.sky},
  add:     {from:"#818CF8", to:"#A78BFA", active:C.violet},
  todo:    {from:"#34D399", to:"#38BDF8", active:C.mint},
  family:  {from:"#64748B", to:"#334155", active:C.gray600},
};
const tabGrad = id => `linear-gradient(140deg,${TAB_ACCENT[id]?.from||C.sky},${TAB_ACCENT[id]?.to||C.violet})`;

// ─── Unified design tokens ───────────────
const R = { card:16, input:11, btn:12, sheet:20, tag:20 };   // border-radius
const SH = {
  card:   "0 2px 12px rgba(0,0,0,0.07)",
  cardMd: "0 4px 18px rgba(0,0,0,0.09)",
  cardLg: "0 8px 28px rgba(0,0,0,0.11)",
  sheet:  "0 -8px 36px rgba(0,0,0,0.14)",
};
const inputStyle = (extra={}) => ({
  ...
  color:C.gray800, transition:"border-color 0.15s",
  ...extra,
});
const labelStyle = { fontSize:11, fontWeight:800, color:C.gray600, display:"block", marginBottom:5, letterSpacing:0.3 };
...
function shiftTime(v){const t=String(v?.time||"").trim();return t&&t!=="-"?t:"";}
...
const INIT_TODOS=[
  ...
  {id:2,text:"수아 물통 챙기기",done:false,memberId:1,due:"오늘",isRoutine:false},
...
function Tag({label,color,bg}){
  return <span style={{
    ...
    borderRadius:R.tag,background:bg,color,
    display:"inline-block",lineHeight:"17px",letterSpacing:0.2,
  }}>{label}</span>;
}
...
function Sheet({open,onClose,children,title}){
  ...
  return(
    <div style={{position:"absolute",inset:0,zIndex:200,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
      <div style={{position:"absolute",inset:0,background:"rgba(15,23,42,0.48)"}} onClick={onClose}/>
      <div style={{
        position:"relative",background:C.white,
        borderRadius:`${R.sheet}px ${R.sheet}px 0 0`,
        maxHeight:"82%",overflowY:"auto",paddingBottom:28,
        boxShadow:SH.sheet,
      }}>
        <div style={{width:40,height:4,borderRadius:4,background:C.gray200,margin:"12px auto 0",display:"block"}}/>
        {title&&<p style={{
          fontSize:15,fontWeight:900,color:C.gray800,
          margin:"14px 20px 0",letterSpacing:-0.3,
        }}>{title}</p>}
        <div style={{padding:"10px 20px 0"}}>{children}</div>
      </div>
    </div>
  );
}
...
function Toast({msg,onUndo,onDone}){
  return(
    <div style={{
      ...
      background:C.gray800,color:"#fff",borderRadius:14,
      padding:"11px 18px",display:"flex",alignItems:"center",gap:14,
      zIndex:300,boxShadow:"0 6px 24px rgba(0,0,0,0.32)",
      whiteSpace:"nowrap",fontSize:13,fontWeight:600,
    }}>
      <span>{msg}</span>
      {onUndo&&<button onClick={onUndo} style={{
        background:"none",border:"none",color:"#60D8FA",
...
function Btn({label,color,bg,onClick,full,outline,danger}){
  return(
    <button onClick={onClick} style={{
      padding:"12px 14px",borderRadius:R.btn,
      border:outline?`1.5px solid ${color}`:"none",
      background:outline?"transparent":danger?C.redBg:(bg||C.gray100),
      color:color||(danger?C.red:C.gray600),
      fontSize:13,fontWeight:700,cursor:"pointer",
      width:full?"100%":"auto",textAlign:"left",
  ...
  return(
    <div style={{marginBottom:10}}>
      ...
      <p style={{margin:0,fontSize:11,color:selected.length?C.violetDeep:C.gray400,fontWeight:selected.length?800:600}}>
        {selected.length?formatRepeatDays(selected):"루틴 요일을 선택하세요"}
      </p>
    </div>
  );
}
...
function HomeScreen({members,events,setEvents,todos,setTodos,shiftMap,shiftTypes,anniv,goTab}){
  ...
  const todayTodos=todos.filter(t=>!t.done&&!t.skipped&&!t.disabled);
  ...
  function openEventEdit(ev){
    setEditForm({id:ev.id,originalDate:ev.date||TODAY,date:ev.date||TODAY,endDate:ev.endDate||ev.date||TODAY,title:ev.title,timeS:ev.timeS||"",timeE:ev.timeE||"",place:ev.place||"",memberId:ev.memberId||"",repeat:ev.repeat||"",repeatDays:parseRepeatDays(ev.repeat,ev.repeatDays)});
  ...
  function saveEventEdit(){
    ...
    setEvents(p=>{
      const fromDate=editForm.originalDate||editForm.date;
      const source=(p[fromDate]||[]);
      const current=source.find(e=>e.id===editForm.id);
      if(!current)return p;
      const updated={...current,title:editForm.title,date:editForm.date,timeS:editForm.timeS,timeE:editForm.timeE,place:editForm.place,memberId:editForm.memberId?+editForm.memberId:null,endDate:current.isRoutine?(editForm.endDate||editForm.date):current.endDate,repeat:current.isRoutine?repeat:current.repeat,repeatDays:current.isRoutine?editForm.repeatDays:current.repeatDays};
      const next={...p,[fromDate]:source.filter(e=>e.id!==editForm.id)};
      next[editForm.date]=[...(fromDate===editForm.date?next[editForm.date]:p[editForm.date]||[]),updated];
      return next;
    });
    setSheet(null);
  ...
  function disableRoutine(id){setTodos(p=>p.map(t=>t.id===id?{...t,disabled:true}:t));setSheet(null);showToast("루틴 체크를 껐습니다.");}
  ...
  function EventSheet({ev}){
    ...
    return(<>
      <div style={{background:C.gray50,borderRadius:14,padding:"12px 14px",marginBottom:14}}>
        ...
        {ev.repeat&&<p style={{margin:"2px 0 0",fontSize:12,color:C.gray400}}>루틴: {ev.repeat}</p>}
      </div>
      <Btn label="✏️ 수정" color={C.skyDeep} bg={C.skyBg} full onClick={()=>openEventEdit(ev)}/>
      {ev.isRoutine?(
        <Btn label="🗑 루틴 삭제" color={C.red} bg={C.redBg} full onClick={()=>setSheet({type:"routineDel",data:ev})}/>
      ):(
  ...
  function EventEditSheet(){
    return(<>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={saveEventEdit} style={{flex:2,padding:10,borderRadius:12,background:editForm.title?tabGrad("home"):C.gray200,border:"none",color:editForm.title?"#fff":C.gray400,fontSize:13,fontWeight:800,cursor:editForm.title?"pointer":"not-allowed"}}>저장</button>
      </div>
    </>);
  }
  ...
  function RoutineDelSheet({ev}){
    return(<>
      ...
      {[
        ...
        {l:"🔁 루틴 수정",sub:"요일과 시간 변경",c:C.violet,bg:C.violetBg,fn:()=>openEventEdit(ev)},
        {l:"🗑 루틴 전체 삭제",sub:"모든 루틴 일정 삭제",c:C.red,bg:C.redBg,fn:()=>deleteEvent(ev)},
      ].map((item,i)=>(
  ...
  function saveTodoEdit(){
    setTodos(p=>p.map(t=>t.id===editForm.id?{...t,text:editForm.text,memberId:editForm.memberId?+editForm.memberId:null,due:editForm.due,isRoutine:false,repeat:undefined,repeatDays:undefined}:t));
    setSheet(null);
    showToast("수정했습니다.");
  }
  ...
  function TodoEditSheet(){
    return(<>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={saveTodoEdit} style={{flex:2,padding:10,borderRadius:12,background:editForm.text?tabGrad("home"):C.gray200,border:"none",color:editForm.text?"#fff":C.gray400,fontSize:13,fontWeight:800,cursor:editForm.text?"pointer":"not-allowed"}}>저장</button>
      </div>
    </>);
  }
  ...
  function EventCard({ev}){
    ...
    return(
      <div onClick={()=>setSheet({type:"event",data:ev})}
        ...
        marginBottom:6,background:ev.isRoutine?(m?.bg||C.violetBg):C.gray50,borderRadius:12,cursor:"pointer",
        borderLeft:`3px solid ${m?m.color:C.gray200}`}}>
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
            <span style={{fontSize:13,fontWeight:700,color:C.gray800}}>{ev.title}</span>
            {ev.isRoutine&&<Tag label="루틴" color={C.mintDeep} bg={C.mintBg}/>}
  ...
  function TodoCard({todo}){
    ...
    return(
      <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",
        marginBottom:6,background:C.gray50,borderRadius:12}}>
        <button onClick={()=>checkTodo(todo.id)} style={{width:22,height:22,borderRadius:"50%",
          border:`2px solid ${C.gray200}`,background:"none",
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",paddingBottom:80,position:"relative"}}>
      ...
      <Sheet open={!!sheet} onClose={()=>setSheet(null)}
        title={sheet?.type==="event"?"일정 상세":sheet?.type==="eventEdit"?"일정 수정":sheet?.type==="routineDel"?"루틴 관리":sheet?.type==="confirmDel"?"삭제 확인":sheet?.type==="todo"?"할 일 상세":sheet?.type==="todoEdit"?"할 일 수정":""}>
      ...
      {/* Hero */}
      <div style={{background:tabGrad("home"),padding:"18px 18px 16px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{padding:"10px 14px 0"}}>
        {/* 오늘 일정 */}
        <div style={{background:C.white,borderRadius:16,padding:"13px 14px",boxShadow:SH.cardMd,marginBottom:10}}>
        ...
        {/* 오늘 할 일 */}
        <div style={{background:C.white,borderRadius:16,padding:"13px 14px",boxShadow:SH.card,marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
            <span style={{fontSize:13,fontWeight:800,color:C.gray800}}>오늘 할 일</span>
          ...
          {todayTodos.map(t=><TodoCard key={t.id} todo={t}/>)}
          {doneTodos.length>0&&(
            <>
...
function CalendarScreen({members,events,setEvents,shiftMap,setShiftMap,shiftTypes,anniv}){
  ...
  const cells=rawCells.concat(Array(Math.max(42,Math.ceil(rawCells.length/7)*7)-rawCells.length).fill(null));
  const shiftMem=members.find(m=>m.isShift)||null;
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",paddingBottom:80}}>
      <div style={{background:tabGrad("calendar"),padding:"14px 16px 12px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,minmax(0,1fr))",gridAutoRows:54,background:C.gray50,padding:"2px 6px 6px",gap:2}}>
        {cells.map((day,idx)=>{
          if(!day)return<div key={"e"+idx} style={{minHeight:54}}/>;
          ...
          return(
            <div key={"d"+day} onClick={()=>{setSelected(ds);setMode("view");}} style={{borderRadius:8,cursor:"pointer",height:"100%",minWidth:0,padding:"4px 2px 3px",background:isSel?tabGrad("calendar"):isToday?C.skyBg:C.white,border:isToday&&!isSel?`1.5px solid ${C.sky}`:"1px solid transparent",overflow:"hidden",display:"flex",flexDirection:"column",justifyContent:"flex-start"}}>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:2,justifyContent:"center",minHeight:6,overflow:"hidden"}}>
      ...
      <div style={{padding:"10px 14px 0"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          ...
          <div style={{display:"flex",gap:6}}>
            ...
            <button onClick={()=>setMode(mode==="addEvent"?"view":"addEvent")} style={{width:28,height:28,borderRadius:"50%",border:"none",cursor:"pointer",background:tabGrad("home"),color:"#fff",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
          </div>
        </div>
          ...
          <div style={{background:C.white,borderRadius:14,padding:14,marginBottom:10,boxShadow:"0 3px 14px rgba(0,0,0,0.08)",border:`1.5px solid ${C.violet}`}}>
            ...
            {selST&&(
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,background:C.gray50,borderRadius:12,padding:"9px 10px",marginBottom:10}}>
                <span style={{fontSize:12,fontWeight:800,color:C.gray600}}>현재 {shiftLabel(selST)}{shiftTime(selST)?` · ${shiftTime(selST)}`:""}</span>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
              {Object.entries(shiftTypes).filter(([,v])=>v.enabled!==false).map(([k,v])=>(
                <button key={k} onClick={()=>selectShift(k)} style={{padding:"10px 0",borderRadius:12,border:"none",background:v.bg,cursor:"pointer",textAlign:"center"}}>
                  ...
                  {shiftTime(v)&&<p style={{margin:"2px 0 0",fontSize:8,color:v.color,opacity:0.7}}>{shiftTime(v)}</p>}
                </button>
              ))}
            </div>
        ...
        {mode==="addEvent"&&(
          <div style={{background:C.white,borderRadius:14,padding:14,marginBottom:10,boxShadow:"0 3px 14px rgba(0,0,0,0.08)",border:`1.5px solid ${C.sky}`}}>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={saveEvent} style={{flex:2,padding:10,borderRadius:10,background:evForm.title?tabGrad("home"):C.gray200,border:"none",color:evForm.title?"#fff":C.gray400,fontSize:12,fontWeight:800,cursor:evForm.title?"pointer":"not-allowed"}}>저장</button>
            </div>
          </div>
        )}
        {selEvs.length===0&&mode==="view"
          ...
          :selEvs.map((ev,i)=>{const m=members.find(x=>x.id===ev.memberId);const ts=ev.timeS?(ev.timeE?`${ev.timeS}~${ev.timeE}`:ev.timeS):"종일";return(
            <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",marginBottom:6,background:ev.isRoutine?(m?.bg||C.violetBg):C.white,borderRadius:12,borderLeft:`3px solid ${m?m.color:C.gray200}`,boxShadow:"0 1px 4px rgba(0,0,0,0.05)"}}>
              <div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}><span style={{fontSize:13,fontWeight:700,color:C.gray800}}>{ev.title}</span>{ev.isRoutine&&<Tag label="루틴" color={C.mintDeep} bg={C.mintBg}/>}</div><span style={{fontSize:11,color:C.gray400}}>{ts}{ev.place?` · ${ev.place}`:""}</span></div>
              {m&&<ZBadge member={m} size={28} bordered={false}/>}
            </div>
          );})}
      </div>
...
function TodoScreen({members,todos,setTodos}){
  ...
  const visible=subTab==="today"?todayAll:subTab==="upcoming"?upcoming:done;
  ...
  function openEdit(todo){
    setEditForm({
      ...
      due:todo.due||"오늘",isRoutine:false,
      repeatDays:[],
      alert:!!todo.alert,
    });
  ...
  function saveEdit(){
    setTodos(p=>p.map(t=>t.id===editForm.id?{
      ...
      isRoutine:false,due:editForm.due,
      repeat:undefined,repeatDays:undefined,
  ...
  function EmptyState(){
    const label=subTab==="today"?"오늘 할 일이 없어요":subTab==="upcoming"?"예정된 할 일이 없어요":"완료된 할 일이 없어요";
  ...
  function EditSheet(){
    return(<>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap"}}>
        {["오늘","내일","이번 주","날짜 없음"].map(v=><button key={v} onClick={()=>setEditForm(p=>({...p,due:v==="날짜 없음"?"":v,isRoutine:false,repeat:undefined,repeatDays:undefined}))} style={{padding:"6px 10px",borderRadius:20,border:"1.5px solid",borderColor:(editForm.due||"날짜 없음")===v?C.mint:C.gray200,background:(editForm.due||"날짜 없음")===v?C.mintBg:C.white,color:(editForm.due||"날짜 없음")===v?C.mintDeep:C.gray600,fontSize:11,fontWeight:800,cursor:"pointer"}}>{v}</button>)}
      </div>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <button onClick={saveEdit} disabled={!editForm.text?.trim()} style={{flex:2,padding:10,borderRadius:12,background:editForm.text?.trim()?tabGrad("todo"):C.gray200,border:"none",color:editForm.text?.trim()?"#fff":C.gray400,fontSize:13,fontWeight:800,cursor:editForm.text?.trim()?"pointer":"not-allowed"}}>저장</button>
      </div>
    </>);
  }
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",paddingBottom:80,position:"relative"}}>
      ...
      <div style={{background:tabGrad("todo"),padding:"16px 18px 18px"}}>
        <h3 style={{color:"#fff",margin:"0 0 12px",fontSize:18,fontWeight:800}}>할 일</h3>
        <div style={{display:"flex",gap:6}}>
          {[{id:"today",l:"오늘"},{id:"upcoming",l:"예정"},{id:"done",l:"완료"}].map(t=>(
      ...
      <div style={{padding:14}}>
        <div style={{display:"flex",gap:6,marginBottom:12,background:C.white,borderRadius:16,padding:"12px 12px",boxShadow:"0 2px 10px rgba(0,0,0,0.06)",alignItems:"center",width:"100%",maxWidth:"100%",overflow:"hidden"}}>
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&add()} placeholder="새 할 일 추가..." style={{flex:"1 1 auto",minWidth:0,border:"none",outline:"none",fontSize:13,background:"transparent",fontFamily:"inherit"}}/>
          <div style={{display:"flex",alignItems:"center",gap:3,flex:"0 1 auto",minWidth:0,overflow:"hidden",justifyContent:"flex-end"}}>
            {familyOrder.map(m=>{
              const sel=quickMember===String(m.id);
              return(
                <button key={m.id} onClick={()=>setQuickMember(sel?"":String(m.id))} title={m.name} style={{
                  width:22,height:22,borderRadius:"50%",border:`1.5px solid ${sel?m.color:C.gray200}`,
                  background:sel?m.bg:C.white,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:14,lineHeight:1,padding:0,boxShadow:sel?`0 0 0 2px ${m.bg}`:"none",flexShrink:0,
                }}>{getZ(m.zodiac).e}</button>
              );
            })}
          </div>
          <button onClick={add} style={{width:32,height:32,borderRadius:"50%",background:tabGrad("todo"),border:"none",color:"#fff",fontSize:19,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,lineHeight:1}}>+</button>
        </div>
...
function AnnivScreen({members,anniv,setAnniv,embedded=false,onBack}){
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",paddingBottom:embedded?10:80}}>
      ...
      <div style={{background:embedded?C.white:"linear-gradient(150deg,#FBBF24,#FB923C)",padding:embedded?"14px 14px 8px":"18px 18px 20px",borderRadius:embedded?16:0,margin:embedded?0:0}}>
...
function AddScreen({members,setEvents,setTodos,anniv,setAnniv,shiftTypes,shiftMap,setShiftMap,goTab}){
  ...
  const [form,setForm]=useState({
    title:"",date:TODAY,endDate:TODAY,timeS:"",timeE:"",place:"",memberId:"",
    repeat:"평일",repeatDays:["월","화","수","목","금"],
    repeatEvent:false,
    month:"",day:"",year:"",shiftMemberId:"",shiftKey:""
  });
  ...
  function save(){
    if((kind==="event"||kind==="todo")&&!form.title.trim())return;
    if(kind==="event"){
      const repeat=form.repeatEvent?(form.repeatDays?.length?formatRepeatDays(form.repeatDays):"요일 선택"):undefined;
      const baseId=Date.now();
      const ev={
        id:baseId,title:form.title,date:form.date,endDate:form.repeatEvent?(form.endDate||form.date):undefined,timeS:form.timeS,timeE:form.timeE,
        place:form.place,memberId:form.memberId?+form.memberId:null,
        isRoutine:form.repeatEvent,repeat,repeatDays:form.repeatEvent?form.repeatDays:undefined,
      };
      setEvents(p=>({...p,[form.date]:[...(p[form.date]||[]),ev]}));
      goTab("calendar");
    }
    if(kind==="todo"){
      setTodos(p=>[...p,{
        id:Date.now(),text:form.title,done:false,memberId:form.memberId?+form.memberId:null,
        due:"오늘",isRoutine:false,
      }]);
      goTab("todo");
    }
    if(kind==="shift"){
  ...
  const canSave=kind==="shift"?!!(form.date&&(form.shiftMemberId||shiftMembers[0])&&(form.shiftKey||activeShiftTypes[0])):!!form.title;

  return(
    <div style={{height:"100%",overflowY:"auto",paddingBottom:80}}>
      <div style={{background:tabGrad("add"),padding:"18px 18px 20px"}}>
        <h3 style={{color:"#fff",margin:"0 0 4px",fontSize:18,fontWeight:800}}>등록</h3>
        <p style={{color:"rgba(255,255,255,0.75)",margin:0,fontSize:12}}>일정·할 일·기념일·근무를 한 곳에서 추가</p>
      </div>
      <div style={{padding:14}}>
          ...
          {[
            ...
            return(
              <button key={id} onClick={()=>setKind(id)} style={{
                minWidth:0,padding:"9px 4px",borderRadius:14,border:"none",
                background:active?tabGrad(id==="anniv"?"family":id==="shift"?"calendar":id):C.white,
                color:active?"#fff":C.gray600,boxShadow:active?"0 8px 18px rgba(59,130,246,0.22)":SH.card,
                cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4,
              }}>
                <span style={{fontSize:17,lineHeight:1}}>{icon}</span>
                <span style={{fontSize:11,fontWeight:900,lineHeight:1.1,whiteSpace:"nowrap"}}>{l}</span>
              </button>
            );
          })}
        </div>
        {kind==="anniv" ? (
          <AnnivScreen members={members} anniv={anniv} setAnniv={setAnniv} embedded/>
        ) : (
        <div style={{background:C.white,borderRadius:16,padding:16,boxShadow:SH.cardLg}}>
          ...
          {kind==="event"&&(
            <>
              <button onClick={()=>setForm(p=>{const next=!p.repeatEvent;return{...p,repeatEvent:next,endDate:next?(p.endDate||p.date):p.endDate,repeatDays:p.repeatDays?.length?p.repeatDays:["월","화","수","목","금"]};})} style={{width:"100%",padding:"10px 12px",borderRadius:12,border:"1.5px solid",borderColor:form.repeatEvent?C.violet:C.gray200,background:form.repeatEvent?C.violetBg:C.gray50,color:form.repeatEvent?C.violetDeep:C.gray600,fontSize:12,fontWeight:900,cursor:"pointer",marginBottom:10,textAlign:"left"}}>
                루틴 {form.repeatEvent?"ON":"OFF"}
                <span style={{display:"block",fontSize:10,fontWeight:600,marginTop:2,color:form.repeatEvent?C.violetDeep:C.gray400}}>학원, 수업, 운동처럼 같은 요일에 이어지는 일정</span>
              </button>
              {form.repeatEvent&&(
                <>
                  <label style={{fontSize:11,fontWeight:700,color:C.gray600,display:"block",marginBottom:4}}>루틴 요일</label>
          ...
          {kind==="shift"&&(
            <>
              ...
              {existingShiftType&&(
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,background:C.gray50,borderRadius:12,padding:"10px 12px",marginBottom:10}}>
                  <div>
                    ...
                    <p style={{margin:"2px 0 0",fontSize:11,color:existingShiftType.color,fontWeight:800}}>{shiftLabel(existingShiftType)}{shiftTime(existingShiftType)?` · ${shiftTime(existingShiftType)}`:""}</p>
                  </div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:12}}>
                {activeShiftTypes.map(([k,v])=>{const sel=(form.shiftKey||activeShiftTypes[0]?.[0])===k;return <button key={k} onClick={()=>setForm(p=>({...p,shiftKey:k}))} style={{padding:10,borderRadius:12,border:`1.5px solid ${sel?v.color:"transparent"}`,background:v.bg,cursor:"pointer",textAlign:"left"}}><p style={{margin:0,fontSize:16,fontWeight:900,color:v.color}}>{shiftLabel(v)}</p>{shiftTime(v)&&<p style={{margin:"2px 0 0",fontSize:10,color:v.color,opacity:0.75}}>{shiftTime(v)}</p>}</button>;})}
              </div>
            </>
          )}
          <button onClick={save} disabled={!canSave} style={{width:"100%",padding:12,borderRadius:12,background:canSave?tabGrad("home"):C.gray200,border:"none",color:canSave?"#fff":C.gray400,fontSize:14,fontWeight:900,cursor:canSave?"pointer":"not-allowed"}}>저장</button>
        </div>
        )}
      </div>
    </div>
...
function FamilyScreen({members,setMembers,shiftTypes,setShiftTypes,anniv,setAnniv,events,todos,shiftMap,setShiftMap,timetable,setTimetable}){
  ...
  return(
    <div style={{height:"100%",overflowY:"auto",paddingBottom:80}}>
      {subPage==="main"&&(<>
        <div style={{background:tabGrad("family"),padding:"18px 18px 22px"}}>
          <h3 style={{color:"#fff",margin:"0 0 4px",fontSize:18,fontWeight:800}}>가족</h3>
          <p style={{color:"rgba(255,255,255,0.6)",margin:0,fontSize:12}}>가족 멤버 · 근무 · 시간표 · 위젯</p>
        </div>
        <div style={{padding:14}}>
            {[
        ...
        <div style={{padding:14,paddingTop:18}}>
          ...
          <div style={{background:C.white,borderRadius:16,padding:16,boxShadow:SH.cardLg}}>
            ...
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClick={saveMember} style={{flex:2,padding:11,borderRadius:12,background:mForm.name?tabGrad("home"):C.gray200,border:"none",color:mForm.name?"#fff":C.gray400,fontSize:13,fontWeight:800,cursor:mForm.name?"pointer":"not-allowed"}}>{editMember?"수정 완료":"추가"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {subPage==="shift"&&(
        <div style={{padding:14,paddingTop:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <BackBtn to="main"/>
            <button onClick={()=>openShiftForm(null)} style={{padding:"5px 14px",borderRadius:20,background:tabGrad("home"),border:"none",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>+ 유형</button>
          </div>
          ...
          {shiftForm&&(
            <div style={{background:C.white,borderRadius:16,padding:16,marginBottom:12,boxShadow:SH.cardLg,border:`1.5px solid ${C.sky}`}}>
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button onClick={saveShiftType} style={{flex:2,padding:10,borderRadius:12,background:tabGrad("home"),border:"none",color:"#fff",fontSize:13,fontWeight:800,cursor:"pointer"}}>저장</button>
              </div>
            </div>
          )}
          ...
          <div style={{background:C.white,borderRadius:16,padding:16,margin:"12px 0",boxShadow:SH.cardLg}}>
                ...
                {shiftEntries().map(x=>(
                  <div key={`${x.memberId}-${x.date}`} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 0",borderTop:`1px solid ${C.gray100}`}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      <p style={{margin:"2px 0 0",fontSize:11,color:C.gray400}}>{x.type?.full||x.key}{shiftTime(x.type)?` · ${shiftTime(x.type)}`:""}</p>
                    </div>
...
// Shared section header — every screen uses this
function ScreenHeader({tabId,title,sub,children}){
  const acc=TAB_ACCENT[tabId]||TAB_ACCENT.home;
  return(
    <div style={{
      background:`linear-gradient(140deg,${acc.from},${acc.to})`,
      padding:"20px 18px 18px",position:"relative",overflow:"hidden",flexShrink:0,
    }}>
      <div style={{position:"absolute",width:110,height:110,borderRadius:"50%",background:"rgba(255,255,255,0.06)",top:-28,right:-28,pointerEvents:"none"}}/>
      {sub&&<p style={{color:"rgba(255,255,255,0.7)",fontSize:11,margin:"0 0 1px",fontWeight:600,letterSpacing:0.4}}>{sub}</p>}
      <h2 style={{color:"#fff",fontSize:20,fontWeight:900,margin:"0 0 0",letterSpacing:-0.5,lineHeight:1.3}}>{title}</h2>
...
export default function App(){
  ...
  const acc=TAB_ACCENT[tab]||TAB_ACCENT.home;

  return(
      <div style={{
        minHeight:"100vh",
        background:"linear-gradient(160deg,#C7E9FF 0%,#E0D7FF 100%)",
      ...
      <style>{`
        ...
        *{box-sizing:border-box;}
        input,button,textarea{font-family:inherit!important;}
      ...
      {/* ── Phone frame ── */}
      <div style={{
        width:375,height:"min(92vh,780px)",
        background:C.page,borderRadius:46,overflow:"hidden",
        boxShadow:"0 36px 90px rgba(0,0,0,0.24),0 0 0 10px #0f0f0f,0 0 0 12px #1d1d1d",
        display:"flex",flexDirection:"column",position:"relative",
      }}>
        {/* Status bar — mirrors active tab accent */}
        <div style={{
          background:`linear-gradient(140deg,${acc.from},${acc.to})`,
          ...
          <span style={{color:"rgba(255,255,255,0.9)",fontSize:12,fontWeight:700}}>9:41</span>
          <div style={{width:114,height:27,background:"#000",borderRadius:22,position:"absolute",left:"50%",transform:"translateX(-50%)",top:7}}/>
          <span style={{color:"rgba(255,255,255,0.9)",fontSize:12}}>🔋</span>
        </div>
        ...
        <div style={{
          ...
          display:"flex",padding:"5px 4px 12px",flexShrink:0,
        }}>
          {TABS.map(t=>{
            ...
            if(isAdd)return(
              <div key={t.id} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <button onClick={()=>setTab(t.id)} style={{
                  width:42,height:42,borderRadius:"50%",border:"none",cursor:"pointer",
                  background:`linear-gradient(140deg,${TAB_ACCENT.add.from},${TAB_ACCENT.add.to})`,
                  display:"flex",alignItems:"center",justifyContent:"center",
                  fontSize:18,marginTop:-6,flexShrink:0,lineHeight:1,
                  boxShadow:active?"0 4px 18px rgba(129,140,248,0.55)":"0 3px 12px rgba(129,140,248,0.35)",
                }}>➕</button>
              </div>
            );
            return(
              <button key={t.id} onClick={()=>setTab(t.id)} style={{
                ...
                display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"5px 0",
              }}>
                <span style={{fontSize:19,lineHeight:1,display:"block",transform:active?"scale(1.18)":"scale(1)",transition:"transform 0.14s"}}>{t.icon}</span>
                <span style={{fontSize:9,fontWeight:active?900:600,lineHeight:1,color:active?ac:C.gray400,transition:"color 0.14s"}}>{t.label}</span>
                {active&&<div style={{width:20,height:3,borderRadius:3,background:`linear-gradient(90deg,${TAB_ACCENT[t.id]?.from||ac},${TAB_ACCENT[t.id]?.to||ac})`,marginTop:1}}/>}
              </button>
            );
          })}
        </div>
