...
- 위험 표시: 같은 구성원에게 배정된 일정의 시간이 실제로 겹칠 때만 월간·상세 행의 작은 빨간 점으로 알린다. 별도 경고 문구와 일정 3개 이상 테두리는 제거했다.
...
- Focused regression: avatar and unified-calendar scenarios 4 passed
...
9. 전체 달력의 시간 겹침은 같은 구성원의 시간대가 실제로 겹칠 때만 감지하며, 상세 행에는 텍스트 대신 카테고리 칩 앞의 빨간 도트로 표시한다.
...
14. 앱 전체 구성원 아바타는 `28×28px`, `8px` 모서리의 둥근 정사각형으로 통일했다. 설정 프로필 아바타도 같은 형태의 시각 언어를 유지한다.
