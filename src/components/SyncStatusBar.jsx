...
export default function SyncStatusBar({ sync }) {
  ...
  if (sync.remoteChange) return (
    <aside className="sync-banner remote" role="status">
      ...
      <button className="dismiss" onClick={sync.dismissRemoteChange} aria-label="알림 닫기"><X /></button>
    </aside>
  )
  ...
  return (
    <aside className={`sync-banner compact ${offline ? 'offline' : saving ? 'saving' : 'synced'}`} role="status">
      {offline ? <CloudOff /> : saving ? <RefreshCw className="spin" /> : sync.syncStatus === 'error' ? <AlertTriangle /> : <Cloud />}
      <span>
        <strong>{offline ? '오프라인 저장 중' : saving ? '가족 일정 동기화 중' : sync.syncStatus === 'error' ? '동기화 확인 필요' : <><Check /> 가족 일정 동기화됨</>}</strong>
        <small>{offline ? '인터넷이 연결되면 이 기기의 변경을 다시 저장합니다.' : sync.error || `마지막 동기화 ${formatSyncTime(sync.lastSyncedAt)}`}</small>
      </span>
    </aside>
  )
}
